...
class PipelineClient:
    ...
    def stop(self):
        ...
        # Notify the callback thread to wake up
        ...
        # Notify the sender thread to wake up from waiting for dirty datapoints
    ...
    def callback_thread_func(self):
        ...
        while self.callback_thread_running and self.running:
            events_batch = []
            ...
            with self.callback_cv:
                ...
                if not self.callback_thread_running or not self.running:
                    break
                
                # Copy events to local batch buffer while holding the lock
    ...
    def process_received_data(self, byte_data: bytearray):
        ...
        Args:
            byte_data: The received data
        """
        logger.debug("Processing {} bytes of received data".format(len(byte_data)))
        # Parse frames from the received data
        success, parsed_frames = BinaryFrameHandler.parse_frame_from_stream(byte_data)
        logger.debug("Frame parsing: success={}, frame_count={}".format(success, len(parsed_frames)))
        
        if not success:
            logger.error("Failed to parse frames from received data")
            return
    ...
    def handle_socket_event(self, event_data: SocketEventData):
        ...
        if event_data.event_type == SocketEvent.CONNECTED:
            ...
            self.process_received_data(bytearray(event_data.data))
