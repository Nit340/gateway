...
def _require_admin_or_webui(request):
    """
    Unified multi-session validation helper.
    Accepts both Admin Portal sessions (gw_admin_session) and Web UI sessions (gw_webui_session).
    Used to protect administrative and diagnostic API endpoints.
    """
    # 1. Try Admin Session
    admin_token = request.cookies.get(_SESSION_COOKIE)
    if admin_token and admin_token in ADMIN_SESSIONS:
        return ADMIN_SESSIONS[admin_token]
        
    # 2. Try Web UI Session
    webui_token = request.cookies.get('gw_webui_session')
    if webui_token and webui_token in WEBUI_SESSIONS:
        session = WEBUI_SESSIONS[webui_token]
        if isinstance(session, dict):
            return session.get('username')
        return session
        
    # 3. Denied
    logger.warning("[AUTH] Access denied for {} - no valid session found".format(request.path))
...
from database import (
    ...
    get_network_settings, update_network_settings,
)
...
from device_management import (
    ...
    get_device_datapoints, update_device_status_api, get_port_config_api, update_port_config_api,
)
...
async def admin_root(request):
    _require_admin_or_webui(request)
...
async def start_background_tasks(app):
    ...
    # --- Network Monitor Background Process ---
    async def _start_network_monitor():
        import sys
        _script = os.path.join(os.path.dirname(__file__), "network.py")
        if not os.path.exists(_script):
            logger.warning("[MAIN] network.py not found at {}".format(_script))
            return

        while True:
            # Check if network monitor is enabled in database
            try:
                cfg = get_network_settings()
                if not cfg.get("enabled", False):
                    logger.debug("[MAIN] Network monitor is disabled in settings. Waiting 30s...")
                    await asyncio.sleep(30)
                    continue
            except Exception as e:
                logger.error("[MAIN] Error checking network settings: {}".format(e))
                await asyncio.sleep(30)
                continue

            logger.info("[MAIN] Starting network monitor process...")
            try:
                # Run network.py as a separate process
                ...
                app["network_monitor_process"] = proc
                ...
                async def _drain_err(stream):
                    while True:
                        line = await stream.readline()
                        if not line: break
                        logger.warning("[NETMON] " + line.decode(errors='replace').rstrip())
                
                asyncio.ensure_future(_drain(proc.stdout))
                asyncio.ensure_future(_drain_err(proc.stderr))
                
                # Wait for it to finish (it shouldn't)
                retcode = await proc.wait()
                logger.warning("[MAIN] Network monitor exited with code {}. Restarting in 5s...".format(retcode))
            except Exception as e:
                logger.error("[MAIN] Failed to start network monitor: {}".format(e))
            
            await asyncio.sleep(5)

    app["network_monitor_task"] = asyncio.ensure_future(_start_network_monitor())

async def cleanup_background_tasks(app):
    logger.info("[MAIN] Cleaning up background tasks...")
    # Kill network monitor process
    proc = app.get("network_monitor_process")
    if proc:
        try:
            proc.terminate()
            logger.info("[MAIN] Terminated network monitor process")
        except Exception:
            pass

    for key in ('network_monitor_task', 'startup_auto_send', 'pipeline_watchdog', 'webui_session_watchdog'):
        task = app.get(key)
        if task:
            task.cancel()
            try:
...
def create_app():
    ...
    async def security_headers_middleware_factory(app, handler):
        ...
        async def security_headers_middleware(request):
            # ── Touch last_ping for any authenticated /api/ request ──────
            # This keeps sessions alive during normal usage regardless of
            # which network interface the request arrived on.  Without this,
            ...
            if request.path.startswith('/api/') and request.path not in _PASSIVE_PATHS:
                try:
                    ...
                    if _tok and _tok in WEBUI_SESSIONS:
                        _sess = WEBUI_SESSIONS[_tok]
                        if isinstance(_sess, dict):
                            _sess['last_ping'] = _time.time()
                except Exception:
                    pass
    ...
    async def api_save_factory_default(request):
        ...
        try:
            ...
            # In WAL mode, uncommitted data lives in the -wal sidecar file and
            # may not yet be flushed into the main .db file. Without this step,
            # shutil.copyfile produces a stale/incomplete snapshot.
            conn = get_db_connection()
            conn.execute('PRAGMA wal_checkpoint(TRUNCATE)')

            factory_db = os.path.join(os.path.dirname(DB_FILE), 'factory_default.db')
            shutil.copyfile(DB_FILE, factory_db)
            logger.info("[FACTORY] Saved factory default DB to {}".format(factory_db))
            return web.json_response({'success': True})
        except Exception as e:
            logger.error("[FACTORY] Save factory default error: {}".format(e))
            return web.json_response({'success': False, 'error': str(e)}, status=500)
    app.router.add_post('/api/admin/save-factory-default', api_save_factory_default)

    async def api_factory_reset(request):
        import shutil, os
        from database import DB_FILE, close_all_connections
        try:
            factory_db = os.path.join(os.path.dirname(DB_FILE), 'factory_default.db')
            if not os.path.exists(factory_db):
                return web.json_response(
                    {'success': False, 'error': 'No factory default saved yet. Use "Save Factory DB" first.'},
                    status=400
                )

            # Close all live connections cleanly so SQLite releases locks.
            close_all_connections()

            # Remove the main DB and its WAL sidecar files.
            # Leaving stale -wal / -shm files would cause SQLite to replay
            # those old transactions on top of the restored snapshot,
            # corrupting the reset result.
            for ext in ('', '-wal', '-shm'):
                p = DB_FILE + ext
                if os.path.exists(p):
                    os.remove(p)

            # Restore from the saved factory copy.
            shutil.copyfile(factory_db, DB_FILE)
            
            # Run initialization on the restored DB to recreate any newer tables 
            # (like external_device) that may be missing from an older factory_default.db
            from database import init_database
            init_database()
            
            logger.info("[FACTORY] Factory reset complete. Restored from {}".format(factory_db))
            return web.json_response({'success': True})
        except Exception as e:
            logger.error("[FACTORY] Factory reset error: {}".format(e))
            return web.json_response({'success': False, 'error': str(e)}, status=500)
    app.router.add_post('/api/admin/factory-reset', api_factory_reset)
    ...
    app.router.add_post('/api/port-config', update_port_config_api)
    ...
    async def api_network_settings_get(request):
        """GET /api/network/settings -- get network monitor configuration from DB"""
        _require_admin_or_webui(request)
        return web.json_response(get_network_settings())

    async def api_network_settings_put(request):
        """PUT /api/network/settings -- update network monitor configuration in DB"""
        _require_admin_or_webui(request)
        try:
            body = await request.json()
            ok = update_network_settings(body)
            return web.json_response({'success': ok})
        except Exception as e:
            return web.json_response({'success': False, 'error': str(e)}, status=500)

    app.router.add_get('/api/network/settings', api_network_settings_get)
    app.router.add_put('/api/network/settings', api_network_settings_put)

    async def api_network_log_file(request):
        level = request.rel_url.query.get('level', '')
        log_path = os.environ.get('GATEWAY_LOG_FILE', '/mnt/data/network_logs.txt')
        try:
            limit = int(request.rel_url.query.get('limit', 200))
            with open(log_path, 'r') as f:
                lines = f.readlines()[-limit:]
            if level and level.upper() not in ('ALL LEVELS', 'ALL', ''):
                lines = [l for l in lines if level.upper() in l]
            return web.json_response({'logs': [l.rstrip() for l in lines]})
        except Exception as e:
            return web.json_response({'logs': [], 'error': str(e)})

    app.router.add_get('/api/network/logs', api_network_log_file)
    
    async def api_terminal_exec(request):
        """POST /api/terminal/exec -- execute shell command"""
        is_admin = False
        
        # 1. Try Admin Portal Session
        admin_token = request.cookies.get(_SESSION_COOKIE)
        if admin_token and admin_token in ADMIN_SESSIONS:
            is_admin = True
            
        # 2. Try Web UI Session with Admin Role
        if not is_admin:
            webui_token = request.cookies.get('gw_webui_session')
            if webui_token and webui_token in WEBUI_SESSIONS:
                sess = WEBUI_SESSIONS[webui_token]
                username = sess.get('username') if isinstance(sess, dict) else sess
                try:
                    conn = get_db_connection()
                    cursor = conn.cursor()
                    cursor.execute('SELECT role FROM webui_users WHERE username=?', (username,))
                    row = cursor.fetchone()
        ...
        if not is_admin:
            return web.json_response({'error': 'Admin access required'}, status=401)
        ...
        try:
            ...
            return web.json_response({'output': stdout.decode(errors='replace')})
        except asyncio.TimeoutError:
            return web.json_response({'error': 'Command timed out'})
        except Exception as e:
            return web.json_response({'error': str(e)}, status=500)

    app.router.add_post('/api/terminal/exec', api_terminal_exec)
