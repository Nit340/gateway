...
import database
from database import get_service_by_name, get_port_config, update_port_config, get_port_label, get_port_value
...
def get_db_connection():
    ...
    conn = sqlite3.connect(database.DB_FILE, timeout=10.0)
...
async def duplicate_device(request):
    ...
    try:
        ...
        if loadcell_row:
            ...
            # Determine available channel if single_ended
            new_device_path = old_device.get('device_path')
            if old_device.get('lc_mode') == 'single_ended':
                # Fixed channels list
                channels = [
                    '/sys/bus/iio/devices/iio:device0/in_voltage0_raw',
                    '/sys/bus/iio/devices/iio:device1/in_voltage0_raw'
                ]
                # Find which one is used by the old device
                used_by_old = old_device.get('device_path')
                # Pick the other one
                for ch in channels:
                    if ch != used_by_old:
                        new_device_path = ch
...
async def export_devices_csv(request):
    ...
    try:
        ...
        writer.writerow([
            ...
            'IP Address', 'TCP Port', 'Port Name', 'Slave ID',
            'LC Mode', 'Poll MS', 'Capacity Min', 'Capacity Max', 'Unit',
            'Deadband', 'Overload', 'Enabled'
        ])
        
        # Export Loadcell devices
        try:
            cursor.execute('SELECT * FROM loadcell_device ORDER BY id')
            for row in cursor.fetchall():
                try:
                    d = {k: row[k] for k in row.keys()}
                    port_label = get_port_label('loadcell', d.get('device_path'))
                    writer.writerow([
                        d.get('id', ''), d.get('name', ''), 'Loadcell', 'loadcell', '',
                        '', '', port_label, '',
                        d.get('lc_mode') or 'single_ended', d.get('poll_ms', 10), 
                        d.get('capacity_min', 0), d.get('capacity_max', 1000), d.get('unit') or 'kg',
                        d.get('deadband', 0), d.get('overload', 0), '1' if d.get('enabled', 1) else '0'
                    ])
                except Exception as row_err:
                    logger.error("Error exporting loadcell row: {}".format(row_err))
        except Exception as e:
            logger.error("Error querying loadcell devices: {}".format(e))

        # Export External devices
        try:
            cursor.execute('SELECT * FROM external_device ORDER BY id')
            for row in cursor.fetchall():
                try:
                    d = {k: row[k] for k in row.keys()}
                    ...
                    port_label = ''
                    if proto == 'ext-rtu':
                        port_label = get_port_label('external', d.get('serial_port'))
                    
                    writer.writerow([
                        d.get('id', ''), d.get('name', ''), d.get('device_type') or 'External', proto, d.get('model_name') or '',
                        d.get('ip_address') or '', d.get('port') or '', port_label, d.get('slave_id') or 1,
                        '', '', '', '', '', # LC specific
                        '', '', # LC specific
                        '1' if d.get('enabled', 1) else '0'
                    ])
                except Exception as row_err:
                    logger.error("Error exporting external row: {}".format(row_err))
        except Exception as e:
            logger.error("Error querying external devices: {}".format(e))
            pass
        ...
        for row_num, row in enumerate(csv_reader, start=2):
            try:
                ...
                if device_type.lower() == 'loadcell':
                    ...
                    # Get mode and resolve port label to path
                    lc_mode = row.get('LC Mode', 'single_ended').strip()
                    port_name = (row.get('Port Name') or row.get('Serial Port') or '').strip()
                    device_path = get_port_value('loadcell', port_name) or '/sys/bus/iio/devices/iio:device0/in_voltage0_raw'
                    
                    cursor.execute('''
                        INSERT INTO loadcell_device (
                            id, name, service_id, device_path, lc_mode,
                            poll_ms, capacity_min, capacity_max, unit,
                            load_name, capacity_name, enabled,
                            deadband, overload
                        )
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    ''', (
                        ...
                        float(row.get('Capacity Min', 0) or 0),
                        float(row.get('Capacity Max', 1000) or 1000),
                    ...
                    # Resolve port name (label) to path
                    port_name = (row.get('Port Name') or row.get('Serial Port') or '').strip()
                    serial_port = get_port_value('external', port_name) or '/dev/ttymxc5'
                    
                    cursor.execute('''
                        INSERT INTO external_device (
                            id, name, protocol, device_type, model_name,
                            slave_id, serial_port, ip_address, port, enabled
                        )
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    ''', (
                        device_id, name, protocol,
                        device_type_val, model_name,
                        int(row.get('Slave ID', 1) or 1),
                        serial_port if 'rtu' in protocol else None,
                        row.get('IP Address', '') if 'tcp' in protocol else None,
                        int(row.get('TCP Port', 502) or 502) if 'tcp' in protocol else None,
                        1 if enabled else 0
                    ))
...
async def update_port_config_api(request):
    """POST /api/port-config  - update a port configuration entry."""
    _require_webui_session(request)
    try:
        data = await request.json()
        port_id = data.get('id')
        if not port_id:
            return web.json_response({'success': False, 'message': 'Port ID is required'}, status=400)
            
        success = update_port_config(port_id, data)
        if success:
            return web.json_response({'success': True, 'message': 'Port configuration updated successfully'})
        else:
            return web.json_response({'success': False, 'message': 'Port configuration not found or no changes made'}, status=404)
    except Exception as e:
        logger.error('Error updating port config: {}'.format(e))
        return web.json_response({'error': str(e)}, status=500)