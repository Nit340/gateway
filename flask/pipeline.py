...
def rebuild_whitelist(include_raw=False):
    ...
    try:
        ...
        _conn = _sq.connect(database.DB_FILE)
    ...
    try:
        ...
        cursor.execute('''
            SELECT
                ...
                -- Centralized Port Settings
                pc.baud_rate           AS pc_baud,
                pc.parity              AS pc_parity,
                pc.data_bits           AS pc_data_bits,
                pc.stop_bits           AS pc_stop_bits,
                pc.response_timeout_ms AS pc_resp_timeout,
                pc.byte_timeout_ms     AS pc_byte_timeout,
                pc.max_retries         AS pc_max_retries,
                pc.polling_interval_ms AS pc_poll_interval,
            ...
            LEFT JOIN port_config pc ON pc.port_value = e.serial_port AND pc.device_type = 'external'
            WHERE ed.enabled = 1
              AND e.enabled  = 1
        ...
        for row in rows:
            ...
            if is_tcp:
                ...
                raw_label = r.get('pc_label')
                conn_id = raw_label.replace(' ', '_') if raw_label else (r.get('serial_port') or '/dev/ttyUSB0')
            
            if conn_id not in connection_map:
                if is_tcp:
                    ...
                    # For RTU, we prioritize centralized port_config settings
                    connection_map[conn_id] = {
                        "id":                conn_id,
                        "type":              "rtu",
                        "device":            r.get('serial_port') or '/dev/ttyUSB0',
                        "baud":              r.get('pc_baud')           if r.get('pc_baud')           is not None else (r.get('baud_rate') or 9600),
                        "parity":            r.get('pc_parity')         if r.get('pc_parity')         is not None else (r.get('parity') or 'N'),
                        "dataBits":          r.get('pc_data_bits')      if r.get('pc_data_bits')      is not None else (r.get('data_bits') or 8),
                        "stopBits":          r.get('pc_stop_bits')      if r.get('pc_stop_bits')      is not None else (r.get('stop_bits') or 1),
                        "responseTimeoutMs": r.get('pc_resp_timeout')   if r.get('pc_resp_timeout')   is not None else (r.get('response_timeout_ms') or 100),
                        "byteTimeoutMs":     r.get('pc_byte_timeout')   if r.get('pc_byte_timeout')   is not None else (r.get('byte_timeout_ms') or 100),
                        "maxRetries":        r.get('pc_max_retries')    if r.get('pc_max_retries')    is not None else (r.get('max_retries') or 2),
                        "pollingIntervalMs": r.get('pc_poll_interval')  if r.get('pc_poll_interval')  is not None else (r.get('polling_interval_ms') or 300),
                    }
            asset = {
                "name":          r['tag_name'],
