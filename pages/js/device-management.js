// device-management.js - Complete fixed version with IIFE
(function() {
    'use strict';
    ...
    function renderDevicesTable() {
        ...
        devices.forEach(function(device) {
            ...
            // UPDATED: Removed firmware from table, simplified view
            row.innerHTML = '<td class="p-4">' +
                '<div class="font-medium text-slate-900">' + device.name + '</div>' +
                '<div class="text-xs text-slate-500 mt-1">' + device.address + '</div>' +
                '</td>' +
                '<td class="p-4">' +
                ...
                '</td>' +
                '<td class="p-4">' +
                '<div class="text-sm">' + (device.group || 'None') + '</div>' +
                '</td>' +
                '<td class="p-4">' +
    ...
    // FIXED: View Details now shows complete configuration
    async function showDeviceDetails(deviceId) {
        try {
            ...
            document.getElementById('firmwareVersion').textContent = device.firmware_version || '-';
    ...
    // FIXED: Edit device populates all fields
        ...
        try {
            ...
            if (device.type === 'Modbus TCP' && device.config) {
                setTimeout(() => {
                    ...
                    document.getElementById('modbusTcpPollingInterval').value = device.config.polling_interval || 1000;
                    document.getElementById('modbusTcpTimeout').value = device.config.timeout || 5000;
                    document.getElementById('modbusTcpRetryCount').value = device.config.retry_count || 3;
                }, 100);
            } else if (device.type === 'Modbus RTU' && device.config) {
                setTimeout(() => {
    ...
    function downloadCSVTemplate() {
        const templateData = [
            ['Device ID', 'Device Name', 'Type', 'Address/ID', 'Firmware Version', 'Group', 'Configuration JSON'],
            ['loadcell', 'LoadCell-Front', 'Modbus RTU', 'Slave 01', '1.2.4', 'Safety Sensors', '{"slave_address":1,"baud_rate":9600,"parity":"None","stop_bits":1,"polling_interval":500}'],
            ['boomangle', 'BoomAngleSensor', 'CAN', '0x212', '2.1.0', 'Crane-01', '{"can_id":"0x212","protocol":"CANOpen","bitrate":"500K"}'],
            ['acs', 'ACS-Node-Left', 'Wireless', 'RF:0x09', '1.5.2', 'Safety Sensors', '{"rf_address":"RF:0x09","signal_strength":-70}'],
            ['modbustcp', 'PLC-Main', 'Modbus TCP', '192.168.1.100:502', '2.3.0', 'Crane-01', '{"ip_address":"192.168.1.100","port":502,"slave_address":1,"polling_interval":1000,"timeout":5000,"retry_count":3}'],
            ['', 'New-Device', 'ACS Sensor', 'ACS-001', '1.0.0', 'New Group', '']
        ];
    ...
    function setupEventListeners() {
        ...
        // Save device - FIXED: Added all Modbus TCP fields
        ...
        if (saveDeviceBtn) {
            saveDeviceBtn.addEventListener('click', async function() {
                ...
                if (deviceType === 'modbus-rtu') {
                    ...
                    config = {
                        slave_address: parseInt(slaveAddress) || 1,
                        ...
                        polling_interval: parseInt(document.getElementById('pollingInterval').value) || 500
                    };
                    ...
                    config = {
                        ...
                        slave_address: parseInt(document.getElementById('modbusTcpSlaveAddress').value) || 1,
                        polling_interval: parseInt(document.getElementById('modbusTcpPollingInterval').value) || 1000,
                        timeout: parseInt(document.getElementById('modbusTcpTimeout').value) || 5000,
                        retry_count: parseInt(document.getElementById('modbusTcpRetryCount').value) || 3
                    };
                } else if (deviceType === 'canbus') {
                    address = document.getElementById('canId').value;
                    config = {
                        can_id: address,
                        protocol: document.getElementById('canProtocol').value || 'CANOpen',
                        bitrate: document.getElementById('bitrate').value || '500K'
                    };
                    ...
                    config = {
                        sensor_id: address,
                        sampling_rate: document.getElementById('acsSamplingRate').value || '10 Hz',
                        sensitivity: document.getElementById('acsSensitivity').value || 'Medium',
                        calibration: parseFloat(document.getElementById('acsCalibration').value) || 1.0
                    };
                }
        ...
        // Template download button - UPDATED
