...
function formatParity(parityCode) {
    ...
    async function loadPortConfig() {
        try {
            ...
            if (data.success && data.ports) {
                portConfig.modbus = data.ports.filter(p => p.device_type === 'external');
    ...
    function populatePortDropdowns() {
        ...
        // --- Load Cell Channel Logic ---
        const devicePathEl = document.getElementById('devicePath');
        const availabilityMsgEl = document.getElementById('lc-availability-msg');
        const msgTextEl = document.getElementById('lc-msg-text');
        const diffBlockedEl = document.getElementById('lc-differential-blocked');
        
        if (devicePathEl && portConfig.loadcell.length) {
            const lcDevices = devices.filter(d => d.protocol === 'loadcell');
            const isDifferential = lcDevices.some(d => d.config && d.config.lc_mode === 'differential');
            const usedPaths = lcDevices.map(d => d.config && d.config.device_path);
            
            if (isDifferential) {
                // Case 2: Differential Mode blocks everything
                devicePathEl.innerHTML = '<option value="" disabled selected>Differential Mode Enabled</option>';
                if (diffBlockedEl) diffBlockedEl.classList.remove('hidden');
                if (availabilityMsgEl) availabilityMsgEl.classList.add('hidden');
            } else {
                if (diffBlockedEl) diffBlockedEl.classList.add('hidden');
                
                // Case 1: Single Load Cell Mode
                devicePathEl.innerHTML = portConfig.loadcell.map(p => {
                    const isUsed = usedPaths.includes(p.port_value);
                    const label = isUsed ? `${p.label} (Used)` : p.label;
                    return `<option value="${p.port_value}" ${isUsed ? 'disabled' : ''}>${label}</option>`;
                }).join('');

                // Availability Message
                if (usedPaths.length > 0) {
                    const usedDevice = lcDevices[0]; // Assuming only one exists if we're here
                    const usedCh = portConfig.loadcell.find(p => p.port_value === usedPaths[0]);
                    const availCh = portConfig.loadcell.find(p => !usedPaths.includes(p.port_value));
                    
                    if (availCh && usedCh && availabilityMsgEl && msgTextEl) {
                        msgTextEl.textContent = `${availCh.label} is available. ${usedCh.label} is already used by ${usedDevice.name}`;
                        availabilityMsgEl.classList.remove('hidden');
                    } else if (availabilityMsgEl) {
                        availabilityMsgEl.classList.add('hidden');
                    }
                } else if (availabilityMsgEl) {
                    availabilityMsgEl.classList.add('hidden');
                }
    ...
    async function refreshData() {
        ...
        const refreshIcon = (refreshBtn && refreshBtn.querySelector('i'));
    ...
    function getDeviceAddress(device) {
        ...
        if (protocol === 'ext-rtu') {
            ...
            // Find by port_value (e.g. /dev/ttymxc5)
            const entry = portConfig.modbus.find(p => p.port_value === serialPort);
            if (entry) return entry.label;
            
            // Fallback to hardcoded defaults if DB entry not found
            if (serialPort === '/dev/ttymxc5') return 'Port 1';
            if (serialPort === '/dev/ttymxc2') return 'Port 2';
            return serialPort;
        } 
        ...
        if (protocol === 'loadcell') {
            ...
            if (lcMode === 'single_ended') {
                const devicePath = config.device_path || '/sys/bus/iio/devices/iio:device0/in_voltage0_raw';
                const entry = portConfig.loadcell.find(p => p.port_value === devicePath);
                if (entry) return entry.label;
                
                if (devicePath.includes('iio:device0')) return 'Channel 1';
                if (devicePath.includes('iio:device1')) return 'Channel 2';
                return 'Channel 1';
            } else {
                return 'Channel 1 (Diff)';
            }
        }
    ...
    function openAddDevicePanel() {
        ...
        if (panel) {
            ...
            const lcDevices = devices.filter(d => d.protocol === 'loadcell');
            const loadcellCount = lcDevices.length;
            const isDifferential = lcDevices.some(d => d.config && d.config.lc_mode === 'differential');

            const lcRadio = panel.querySelector('input[name="device-type"][value="loadcell"]');
            const lcLabel = document.getElementById('lc-radio-wrapper');
            const lcLimitBadge = document.getElementById('lc-limit-badge');
            const lcLimitInfo = document.getElementById('lc-limit-info');
            const lcLimitText = document.getElementById('lc-limit-text');

            if (lcRadio) {
                const disabled = loadcellCount >= 2 || isDifferential;
                lcRadio.disabled = disabled;
                
                if (lcLimitBadge) lcLimitBadge.classList.toggle('hidden', !disabled);
                if (lcLimitInfo) lcLimitInfo.classList.toggle('hidden', !disabled);
                
                if (disabled) {
                    let msg = 'Maximum limit of 2 loadcells already connected.';
                    if (isDifferential) {
                        msg = 'A loadcell is already connected in Differential Mode, which uses both channels.';
                    }
                    if (lcLimitText) lcLimitText.textContent = msg;
                    if (lcLabel) {
                        lcLabel.style.opacity = '0.5';
                        lcLabel.style.cursor = 'not-allowed';
                        lcLabel.title = msg;
                    }
                } else {
                    if (lcLabel) {
                        lcLabel.style.opacity = '1';
                        lcLabel.style.cursor = 'pointer';
                        lcLabel.title = '';
                    }
                }
            }
    ...
    function handleDeviceTypeChangeWithLimit(event) {
        ...
        if (selectedValue === 'loadcell' && (loadcellCount >= 2 || isDifferential)) {
            const msg = isDifferential 
                ? 'Load cell is configured in Differential Mode. No additional load cells can be created.'
                : 'Maximum 2 Loadcell devices allowed. Please delete an existing one first.';
    ...
    function switchDeviceType(type) {
        ...
        // Update active class on labels for older browser compatibility
        if (panel) {
            panel.querySelectorAll('.device-type-option').forEach(opt => {
                const radio = opt.querySelector('input[name="device-type"]');
        ...
        try {
            ...
            if (deviceType === 'loadcell') {
                ...
                requestData.config = {
                    ...
                    unit: document.getElementById('lcUnit') ? document.getElementById('lcUnit').value.trim() : 'kg',
                ...
                const extDeviceTypeInit = document.getElementById('extDeviceTypeInit') ? document.getElementById('extDeviceTypeInit').value.trim() : '';
                const extModelName = document.getElementById('extModelName') ? document.getElementById('extModelName').value.trim() : '';
    ...
    function _buildEditConfigFields(proto, cfg) {
        ...
        if (proto === 'ext-rtu') {
            ...
            return `<div class="space-y-4">
                ...
                <p class="text-xs text-slate-400 mt-1 mb-2">This will be displayed as the Address/ID in the table view. Serial settings are managed centrally in <strong>Port Configuration</strong>.</p>
                
                <div class="hidden">
                    <!-- Hidden inputs to maintain compatibility with existing save logic -->
                    <input type="hidden" id="editBaudRate" value="${cfg.baud_rate||9600}">
                    <input type="hidden" id="editDataBits" value="${cfg.data_bits||8}">
                    <input type="hidden" id="editParity" value="${cfg.parity||'N'}">
                    <input type="hidden" id="editStopBits" value="${cfg.stop_bits||1}">
                    <input type="hidden" id="editResponseTimeout" value="${cfg.response_timeout_ms||100}">
                    <input type="hidden" id="editByteTimeout" value="${cfg.byte_timeout_ms||100}">
                    <input type="hidden" id="editMaxRetries" value="${cfg.max_retries||2}">
                    <input type="hidden" id="editPollingInterval" value="${cfg.polling_interval_ms||300}">
                </div>
            </div>`;
        }
        if (proto === 'ext-tcp') {
    ...
    async function saveEditDevice() {
        ...
        try {
            ...
            const editDeviceType = g('editDeviceType') ? g('editDeviceType').value.trim() : '';
            const editModelName = g('editModelName') ? g('editModelName').value.trim() : '';
            ...
            if (g('editUnit')) cfg.unit = g('editUnit').value ? g('editUnit').value.trim() : 'kg';
    ...
    window.deviceManagement = {
        ...
        duplicateDeviceFromTable: async function(deviceId) {
            ...
            if (device && device.protocol === 'loadcell') {
                ...
                if (isDifferential) {
                    showNotification('Cannot duplicate Loadcell: System is in Differential Mode (uses both channels).', 'warning');
                    return;
                }
                if (lcDevices.length >= 2) {
            ...
            const deviceName = device ? device.name : 'this device';
            const confirmed = await showDuplicateDeviceConfirmModal(deviceName, device);
            if (!confirmed) return;
    ...
    function showDuplicateDeviceConfirmModal(deviceName, device = null) {
        return new Promise((resolve) => {
            let loadcellWarningHtml = '';
            if (device && device.protocol === 'loadcell') {
                const usedChPath = device.config && device.config.device_path ? device.config.device_path : '';
                const usedCh = usedChPath.includes('device1') ? 'Channel 2' : 'Channel 1';
                const nextCh = usedCh === 'Channel 1' ? 'Channel 2' : 'Channel 1';
                loadcellWarningHtml = `
                    <div style="background:#FFFBEB;border:1px solid #FDE68A;border-radius:8px;padding:12px;margin-bottom:20px;display:flex;align-items:flex-start;gap:8px;">
                        <i class="fa-solid fa-triangle-exclamation" style="color:#D97706;margin-top:2px;flex-shrink:0;"></i>
                        <p style="margin:0;font-size:13px;color:#92400E;"><strong>Hardware Channel Update:</strong> The new duplicated loadcell will automatically be assigned to <strong>${nextCh}</strong> to prevent hardware conflicts.</p>
                    </div>
                `;
            }
            ...
            overlay.innerHTML = `
                <div style="background:white;border-radius:12px;padding:28px;max-width:460px;width:100%;box-shadow:0 20px 40px rgba(0,0,0,0.2);">
                    ...
                    <div style="background:#F0FDF4;border:1px solid #BBF7D0;border-radius:8px;padding:12px;margin-bottom:20px;display:flex;align-items:flex-start;gap:8px;">
                        ...
                        <p style="margin:0;font-size:13px;color:#166534;">Example: <strong>${escapeHtml(deviceName)}</strong> → <strong>${escapeHtml(deviceName)}-001</strong></p>
                    </div>
                    <div style="display:flex;gap:10px;justify-content:flex-end;">
                        ...
                        <button id="dmDupConfirmBtn" style="padding:9px 18px;background:#3B82F6;border:none;border-radius:8px;font-size:14px;font-weight:500;color:white;cursor:pointer;">Duplicate</button>
                    </div>
                </div>`;
            document.body.appendChild(overlay);
    ...
    function setupImportExportListeners() {
        ...
        if (dropArea && !dropArea.hasListener) {
            dropArea.addEventListener('click', (e) => {
                ...
                if (e.target === dropArea || (e.target !== browseBtn && !(browseBtn && browseBtn.contains(e.target)))) {
                    if (fileInput) fileInput.click();
                }
            });
            ...
            duplicateDeviceBtn.addEventListener('click', async function() {
                if (currentViewingDeviceId) {
                    ...
                    if (device && device.protocol === 'loadcell') {
                        ...
                        if (lcDevices.length >= 2) {
                            showNotification('Cannot duplicate Loadcell: Maximum limit of 2 loadcells reached.', 'warning');
                            return;
                        }
                    }
                    const deviceName = device ? device.name : 'this device';
