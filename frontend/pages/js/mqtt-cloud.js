...
function _bindPageListeners() {
    ...
    _el('exportBtn')?.addEventListener('click', () => _showM('exportModal'));
    _el('closeExportModal')?.addEventListener('click', () => _hideM('exportModal'));
    _el('exportJsonBtn')?.addEventListener('click', () => {
        window.location.href = `${API}/export?format=json`;
        _hideM('exportModal');
        _toast('JSON export started', 'success');
    });
    _el('exportCsvBtn')?.addEventListener('click', () => {
        window.location.href = `${API}/export?format=csv`;
        _hideM('exportModal');
        _toast('CSV export started', 'success');
    });
    _el('importBtn')?.addEventListener('click', () => _el('importFileInput')?.click());
    _el('importFileInput')?.addEventListener('change', async function () {
        ...
        const loader = _el('mqtt-page-loader');
        const mainText = _el('mqtt-loader-main');
        const subText = _el('mqtt-loader-sub');
        let oldMain = mainText ? mainText.textContent : 'Saving Cloud Configuration';
        let oldSub = subText ? subText.textContent : 'Synchronizing settings with the cloud...';
        
        if (loader) {
            if (mainText) mainText.textContent = 'Importing Connections';
            if (subText) subText.textContent = 'Uploading and parsing backup file...';
        ...
        try {
            ...
            _toast(d.message || 'Import successful', 'success');
            await _loadConnections();
        } catch (err) {
            ...
            if (loader) {
                setTimeout(() => {
                    ...
                    if (mainText) mainText.textContent = oldMain;
                    if (subText) subText.textContent = oldSub;
                }, 1000);
            }
        }
    });
    ...
    _el('exportModal')?.addEventListener('click', e => { if (e.target.id==='exportModal') _hideM('exportModal'); });
    document.addEventListener('keydown', e => { if (e.key==='Escape') { _hideM('addConnectionModal'); _hideM('addTagsModal'); _hideM('exportModal'); }});
}
